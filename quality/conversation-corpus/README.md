...
```bash
./quality/conversation-corpus/run-local.sh --repetitions 3
```
...
```bash
ruby ./quality/conversation-corpus/report.rb validate --scenario ./quality/conversation-corpus/scenarios/01-happy-first-contact.yml
```

É possível apontar para outro ingresso local:

```bash
./quality/conversation-corpus/run-local.sh --base-url http://127.0.0.1:8081 --repetitions 3
```

O runner executa cada cenário e grava artefatos ignorados pelo Git em
`quality/conversation-corpus/results/`. Cada execução recebe `runId`, `repetition`, `contactId`,
...
```bash
./quality/conversation-corpus/run-local.sh --memory-seed-mode setup-events
```
