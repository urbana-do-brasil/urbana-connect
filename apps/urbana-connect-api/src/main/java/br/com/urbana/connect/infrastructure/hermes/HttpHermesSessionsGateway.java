...
public class HttpHermesSessionsGateway implements HermesSessionsGateway {
    ...
    public HermesChatResult chat(String sessionId, HermesChatRequest request) {
        ...
        if (content == null || content.isBlank()) {
            throw HermesSessionsException.ambiguous("Hermes chat returned no usable message");
        }
    ...
    public List<HermesHistoryMessage> history(String sessionId) {
        return historySnapshot(sessionId).messages();
    }
    ...
    public HermesHistorySnapshot historySnapshot(String sessionId) {
        ...
        String cursor = text(body, "cursor", "next_cursor", "nextCursor");
        if (cursor == null || cursor.isBlank()) {
            cursor = deriveStableCursor(result);
        }
        return new HermesHistorySnapshot(cursor, result);
    }
    ...
    private ExchangeResponse exchange(String path, Object payload) {
        try {
            ...
            throw HermesSessionsException.fromTransport(exception);
        }
    }
    ...
    private static String deriveStableCursor(List<HermesHistoryMessage> messages) {
        ...
        for (HermesHistoryMessage message : messages) {
            updateDigest(digest, message.role());
            updateDigest(digest, message.content());
        }
    ...
    public enum HermesFailurePhase {
        PRE_DISPATCH,
    ...
    public static final class HermesSessionsException extends RuntimeException {
        ...
        private final HermesFailurePhase phase;
        private final boolean retryAllowed;

        public HermesSessionsException(String message, Throwable cause) {
            this(message, cause, 0, HermesFailurePhase.POST_DISPATCH_AMBIGUOUS, false);
        }

        private HermesSessionsException(String message, Throwable cause, int status,
                                        HermesFailurePhase phase, boolean retryAllowed) {
            super(message, cause);
            this.status = status;
            this.phase = phase;
        ...
        public HermesFailurePhase phase() {
            return phase;
        }
        ...
        public static HermesSessionsException fromStatus(int status, String body) {
            ...
            HermesFailurePhase phase = status >= 500 || status == 429
                    ? HermesFailurePhase.POST_DISPATCH_AMBIGUOUS
                    : HermesFailurePhase.PRE_DISPATCH;
            return new HermesSessionsException(category + " (HTTP " + status + ")", null, status,
                    phase, phase == HermesFailurePhase.PRE_DISPATCH);
        }

        public static HermesSessionsException fromTransport(Throwable cause) {
            HermesFailurePhase phase = hasCause(cause, ConnectException.class)
                    || hasCause(cause, UnknownHostException.class)
                    || hasCause(cause, NoRouteToHostException.class)
                    ? HermesFailurePhase.PRE_DISPATCH
                    : HermesFailurePhase.POST_DISPATCH_AMBIGUOUS;
            return new HermesSessionsException("Hermes Sessions API transport failure", cause, 0,
                    phase, phase == HermesFailurePhase.PRE_DISPATCH);
        }

        public static HermesSessionsException ambiguous(String message) {
            return new HermesSessionsException(message, null, 0,
                    HermesFailurePhase.POST_DISPATCH_AMBIGUOUS, false);
        }

        public static HermesSessionsException preDispatch(String message) {
            return new HermesSessionsException(message, null, 0,
                    HermesFailurePhase.PRE_DISPATCH, true);
        }
