...
COMPOSE_FILE="${COMPOSE_FILE:-$REPO_ROOT/infra/local-poc/docker-compose.poc.yml}"
...
require_file "$APP_ROOT/Dockerfile"
require_file "$APP_ROOT/nginx/nginx.conf"
require_file "$APP_ROOT/nginx/default.conf.template"
require_file "$APP_ROOT/docker-entrypoint.d/10-poc-token.sh"

grep -Eq '^FROM node:24-alpine' "$APP_ROOT/Dockerfile" \
  || fail "Dockerfile has no Node 24 build stage"
grep -Eq '^FROM nginxinc/nginx-unprivileged:' "$APP_ROOT/Dockerfile" \
  || fail "Dockerfile has no unprivileged Nginx runtime"
grep -Eq '^USER 101(:101)?$' "$APP_ROOT/Dockerfile" \
...
grep -Fq 'proxy_set_header Authorization "Bearer ${HERMES_POC_API_TOKEN}";' \
  "$APP_ROOT/nginx/default.conf.template" \
...
grep -Eq 'add_header[[:space:]]+Access-Control-Allow-Origin' "$APP_ROOT/nginx/default.conf.template" \
