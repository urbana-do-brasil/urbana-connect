...
describe('ConversationClient', () => {
  ...
  it('reads the safe turn summary without forwarding backend-only fields', async () => {
    const fetchMock = vi.fn<FetchImplementation>().mockResolvedValue(jsonResponse({
      ...projection(),
      turn: {
        status: 'RECONCILING',
        ...
        finishedAt: null,
        hermesSessionId: 'must-not-reach-ui',
      },
    }));
    const client = new ConversationClient(fetchMock);

    const result = await client.getConversationProjection(ALIAS);
