...
INTEGRATION_DIR="$REPO_ROOT/integrations/hermes-agent"
...
validate_profile_contract() {
  local profile_dir="$INTEGRATION_DIR/profile"
  local plugin_dir="$INTEGRATION_DIR/plugins/urbana-domain"
  ...
  grep -Fq 'texto conversacional' "$profile_dir/SOUL.md" || {
    echo "Hermes profile must describe natural conversational text." >&2
    exit 3
  }
  if grep -Eq 'nextAction|objeto JSON|responda com um único objeto' "$profile_dir/SOUL.md"; then
    echo "Hermes profile must not require a structured conversational envelope." >&2
    exit 3
  fi
}
...
validate_compose_isolation() {
  ...
  grep -Fq '../../integrations/hermes-agent/profile/SOUL.md:/opt/data/SOUL.md:ro' "$COMPOSE_FILE" || {
    echo "POC compose must mount the isolated SOUL.md read-only." >&2
    exit 3
  }
  grep -Fq '../../integrations/hermes-agent/profile/config.yaml.example:/profile/config.yaml:ro' "$COMPOSE_FILE" || {
  ...
  grep -Fq '../../integrations/hermes-agent/plugins/urbana-domain:/opt/data/plugins/urbana-domain:ro' "$COMPOSE_FILE" || {
